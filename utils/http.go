...
func (h *HttpSend) Get() (map[string]interface{}, error) {
	body, _ := h.send(GET_METHOD)
	return ParseBody(body)
}

func (h *HttpSend) Post() (map[string]interface{}, error) {
	body, _ := h.send(POST_METHOD)
	return ParseBody(body)
}
...
func ParseBody(body []byte) (map[string]interface{}, error) {
	...
	err := json.Unmarshal([]byte(body_str), &data)
	if err == nil {
		return data, err
	}
	return nil, err
